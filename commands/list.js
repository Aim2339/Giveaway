...
module.exports.run = async (client, message, args) => {
    ...
    .addOptions([
      {
        ...
        description:
          "Check the normal giveaways currently running in your server!",
        value: "normal",
      },
    ]);
  ...
  const filter = (x) =>
    x.customId == "select" && x.user.id == message.author.id;
      ...
      await Promise.all(
        giveaways.map(async (x) => {
          embed.addFields({
            ...
            value: `**Prize:** **[${x.prize}](https://discord.com/channels/${
              x.guildId
